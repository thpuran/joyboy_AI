...
# ✅ STEP 6.5: Click on first available date (Select / Book)
try:
    date_buttons = driver.find_elements(By.XPATH, "//button[contains(text(),'Select') or contains(text(),'Book')]")
    if date_buttons:
        date_buttons[0].click()  # Click first available date
        print("✅ Selected available date.")
    else:
        print("⚠️ No available date found.")
except Exception as e:
    print(f"❌ Error selecting date: {e}")
time.sleep(2)

# STEP 7: Click SELECT MODULES
try:
...
# STEP 9: Login form – enter credentials
try:
